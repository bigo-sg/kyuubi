...
public class SemanticAnalyzerHiveDriverRunHook implements HiveDriverRunHook {
	...
	private List<String> fetchPartitionKeys(String schemaName, String tableName) throws Exception {
		...
		// System.out.println("table: "+schemaName+"."+tableName+", partitionKeys:
		// "+partitionKeys);
		return partitionKeys;
	}
				...
				if (join.isInner()) {
					...
					for (String joinKey : joinKeys) {
						...
						if (!str.contains(".")) {
							...
							} else if (tbName.equals(rightAlias.toLowerCase())
									|| tbName.equals(rightTableName.toLowerCase())) {
								rightJoinKeys.add(key);
							}
						}
					}
					handleJoin(leftTableName, rightTableName, leftWhereKeys, rightWhereKeys, leftJoinKeys,
							rightJoinKeys);
					...
					// select id, day from vlog.testparser, (select * from (select id,day from
					// vlog.testparser2 where day='')t) as data1；
	...
	private static void handleJoin(String leftTableName, String rightTableName, Set<String> leftWhereKeys,
			Set<String> rightWhereKeys, Set<String> leftJoinKeys, Set<String> rightJoinKeys) throws Exception {
	...
	public static boolean hasPercentile(String sql) throws KyuubiSQLException {
		if (sql.contains("percentile")) {
			if (sql.contains("percentile_approx") || sql.contains("approx_percentile")) {
				return true;
			} else {
				throw new KyuubiSQLException(
						"spark percentile() performance is weak, please use percentile_approx() instead");
			}
		}
		return false;
	}
